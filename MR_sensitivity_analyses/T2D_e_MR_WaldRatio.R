...
for (i in 1:length(cd4_data_files_name)){
  ...
  results_wr$outcome<-"T2D"
  write.csv(results_wr,paste("/PATH_TO_MY_FOLDER/results/T2D/1SNP_Waldratio/",cd4_data_files_name[i],"_T2D.csv",sep=""),row.names=FALSE)
  #results_raps<-mr(cd4_data_2plus,method_list = "mr_raps",parameters = list(over.dispersion=FALSE,loss.function="l2"))
  #write.csv(results_raps,paste("/PATH_TO_MY_FOLDER/results/T2D/2+SNP_RAPS/",cd4_data_files_name[i],"_T2D.csv",sep=""),row.names=FALSE)
}
