...
for (i in 1:length(cd4_data_files_name)){
  ...
  #mr_ivw(robust=TRUE) for 2 or more SNPs
  ...
  for (j in 1:length(cd4_data_2plus_mr)){
    ...
    try(MR_Robust<-mr_ivw(data_for_MR,robust = TRUE),silent = TRUE)
    MR_Robust_results<-cbind(names(cd4_data_2plus_mr[j]),MR_Robust$SNPs,MR_Robust$Estimate,MR_Robust$StdError,MR_Robust$CILower,MR_Robust$CIUpper,MR_Robust$Pvalue)
    write.table(MR_Robust_results,
                paste("/PATH_TO_MY_FOLDER/results/CVD/2+SNPs_MR-Robust_full/",cd4_data_files_name[i],"_MRRobust.txt",sep=""),
